...
@section('content_header')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1 class="m-0">Información de Ingreso:
                        @if ($edicion = 1)
                ...
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        ...
                        <li class="breadcrumb-item active">Formulario Edicion abierta </li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
...
@section('content')
    ...
    //$disabled = '';
    ?>
    {{-- @if ($edicion == 0 || $edicion == '')
        {{ $status_readonly = 'readonly="off"' }}
    ...
    <fieldset>
        <div class="card">
            ...
            <div class="card-body" id="modal-register-item">
                ...
                @foreach ($ingresos as $ingEdit)
                    <div class="row">
                        <div class="col">
                            <label>Nro Ingreso:</label>
                            <input type="text" class="numeroEntero form-control" value="{{ $ingEdit->nro_ingreso }}"
                                id="nro_ingreso" name="nro_ingreso" placeholder="Ingrese el número de ingreso">
                            <input type="hidden" id="id_ingreso" name="id_ingreso"value="{{ $ingEdit->id }}">
                            <input type="hidden" id="cerrar_edicion" name="cerrar_edicion"value="{{ $ingEdit->edicion }}">
                        </div>
                        <div class="col">
                            <label>Nro Egreso:</label>
                            <input class=" numeroEntero form-control" value="{{ $ingEdit->nro_egreso }}"id="nro_egreso"
                                name="nro_egreso" placeholder="Ingrese el número de egreso">
                        </div>
                        <div class="col">
                            <label>Almacen:</label>
                            <select class="form-control" id="almacen" name="almacen" value="{{ $ingEdit->almacen }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-4">
                            ...
                            $fecha_ingreso = date('Y-m-d', strtotime($ingEdit->created_at));
                            ?>
                    ...
                    <div class="row">
                        <div class="col-8">
                            ...
                            <input type="text" class="form-control" id="funcionario" name="funcionario" readonly
                                value="{{ $ingEdit->funcionario }}">
                        </div>
                     
                        <div class="col-4">
                            <label>Estado:</label>
                            <select class="form-control" id="estado_tabla" name="estado" value="{{ $ingEdit->estado }}">
                    ...
                    <div class="row">
                        <div class="col-12">
                            ...
                            <textarea class="form-control" value="{{ $ingEdit->observaciones }}" id="observacion" name="observaciones"
                                aria-label="Wilabel textarea" placeholder="Ingrese una observación">{{ $ingEdit->observaciones }}</textarea>
                        </div>
                    </div>
                @endforeach
                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <button type="button" class="add_item btn btn-primary" id="add_item">
                                <i class="fas fa-plus-circle"> ITEM</i></button>
                            <br />
                            <table class="table table-striped" id="detalleIngresoUpdate">
                                <thead>
                                    <tr>
                                        <th>CODIGO </th>
                                        <th>DESCRIPCION </th>
                ...
                <div class="text-center">
                    <div class="row">
                        <div class="col-12">
                            <button type="submit" id="btn_guardar_ingreso_update"
                                class="btn btn-success mt-4">Actualizar
                                Ingreso</button>

                            <button type="button" id="btn_cerrar_edicion" class="btn btn-warning mt-4">Cerrar
                                Edición</button>

                            <button id="btn_imprimir_ingreso" class="btn btn-secondary mt-4">Imprimir</button>
                            <a href="{{ route('ingreso') }}" id="btn_salir_ingreso"
                                class="btn btn-danger mt-4">Salir</a>
                        </div>
                    </div>
                </div>
                {{-- </form> --}}
                ...
                <div class="modal-body">
                    ...
                    <div class="row">
                        ...
                        <div class="col-6">
                            ...
                            <input type="text" class="form-control datepicker" id="precio_ingreso"
                                name="precio_ingreso" placeholder="ingrese precio" required>
    ...
    <script>
        $(document).ready(function() {
            ...
            if ($("#cerrar_edicion").val() == 0) {
                ...
                $("#orden_compra").attr("readonly", "readonly");
                
                $("#nro_ingreso").attr("readonly", "readonly");
                $("#nro_egreso").attr("readonly", "readonly");
                $("#almacen").attr("readonly", "readonly");

                // $("#almacen").removeAttr("readonly");
                $("#almacen").attr("readonly", "readonly");
                $("#almacen").attr('disabled', true);

                $("#estado_tabla").removeAttr("readonly");
                $("#estado_tabla").attr('disabled', true);
    ...
    <script>
        ...
        $(document).on('click', '#btn_cerrar_edicion', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "post",
                        url: "{{ route('ingreso.cerrarEdicion') }}",
                        data: {
                            _token: "{{ csrf_token() }}",
                        ...
                        success: function(data) {
                            ...
                            $('#detalleIngresoUpdate').DataTable().ajax.reload();
        ...
        $('#agregarItem').on('click', function(event) {
            ...
            if (id_unidad_medida_temp == '') {
                ...
                $.ajax({
                    url: "{{ route('ingreso.guardarItemUpdate') }}",
                    type: 'POST',
                    dataType: 'json',
                    async: false,
                    data: {
                    ...
                    success: function(data_response) {
                        ...
                        table.ajax.reload();
                        $('#modal_add_item').trigger(
                            "reset"); // for cleaning a modal form
        ...
        $('#btn_guardar_ingreso_update').on('click', function() {
            $.ajax({
                ...
                url: "{{ route('ingreso.storeItemsUpdate') }}",
                async: false,
                data: JSON.stringify({
        ...
        function eliminaritem(id) {
            // alert(id);
            $.ajax({
                url: "{{ route('ingreso.deleteItemUpdate') }}",
                type: 'POST',
                dataType: 'json',
                data: {
                    _token: "{{ csrf_token() }}",
                ...
                success: function(data) {
                    // location.reload();
                    var table = $('#detalleIngresoUpdate').DataTable();
                    table.ajax.reload();
        ...
        $(function() {
            $('#detalleIngresoUpdate').DataTable({
                processing: true,
                serverSide: true,
                searching: true,
                paging: true,
                ...
                ajax: {
                    url: "{{ route('ingreso.detalleIngresoUpdate') }}",
                    type: "get",
                    data: {
                        id: $('#id_ingreso').val()
                    },
                    ...
                    },
                    {
                        ...
                        render: function(data, type, full, meta) {
                            ...
                            if ($('#cerrar_edicion').val() == 0) {
                                ...
                                button +=
                                    '<button type="button" id="btn_delete" class="btn btn-danger "  onclick="eliminaritem(' +
                                    data.id + ')">';
